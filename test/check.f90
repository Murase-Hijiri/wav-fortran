program check
  ...
  bitq = 16
  call lpcm_wav_write("./test/demo_sine_16bit.wav", demo_sine, sampling_rate, bitq)
  deallocate(demo_sine)

  ! read the wav file (16bit) and save into a wav file (8bit)

  call lpcm_wav_read("./test/demo_sine_16bit.wav", demo_sine, sampling_rate)
  bitq = 8
  call lpcm_wav_write("./test/demo_sine_8bit.wav", demo_sine, sampling_rate, bitq)
  deallocate(demo_sine)

  block
    integer(int32) :: unit_num
    open(newunit=unit_num, file = "./test/demo_sine_16bit.wav")
    close(unit_num, status="delete")
    open(newunit=unit_num, file = "./test/demo_sine_8bit.wav")
    close(unit_num, status="delete")
  end block
